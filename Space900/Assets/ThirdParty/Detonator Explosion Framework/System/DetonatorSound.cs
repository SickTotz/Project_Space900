    ...
    override public void Explode()
    {
        ...
        if (_explodeDelay <= 0)
        {
            if (Vector3.Distance(Camera.main.transform.position, this.transform.position) < distanceThreshold)
            {
                ...
                AudioSource explosionAudio = gameObject.AddComponent<AudioSource>();
                explosionAudio.clip = nearSounds[_idx];
                explosionAudio.volume = Random.Range(minVolume, maxVolume);
                explosionAudio.Play();
            }
            else
            {
                ...
                AudioSource explosionAudio = gameObject.AddComponent<AudioSource>();
                explosionAudio.clip = farSounds[_idx];
                explosionAudio.volume = Random.Range(minVolume, maxVolume);
                explosionAudio.Play();
            }
